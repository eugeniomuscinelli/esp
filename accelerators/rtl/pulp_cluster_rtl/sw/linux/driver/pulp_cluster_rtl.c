...
static void pulp_cluster_prep_xfer(struct esp_device *esp, void *arg)
{
    ...
    /* <<--regs-config-->> */
	iowrite32be(a->boot_offset, esp->iomem + PULP_CLUSTER_BOOT_OFFSET_REG);
	iowrite32be(a->spare0, esp->iomem + PULP_CLUSTER_SPARE0_REG);
	iowrite32be(a->spare1, esp->iomem + PULP_CLUSTER_SPARE1_REG);
...
static void __exit pulp_cluster_exit(void) { esp_driver_unregister(&pulp_cluster_driver); }

module_init(pulp_cluster_init) module_exit(pulp_cluster_exit)
