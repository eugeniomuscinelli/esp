...
/* <<--params-->> */
const int32_t boot_offset = 0x8080;
const int32_t spare0 = 1;
const int32_t spare1 = 1;
...
int main(int argc, char *argv[])
{
    ...
    if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
        ...
        in_words_adj  = round_up(1024, DMA_WORD_PER_BEAT(sizeof(token_t)));
        out_words_adj = round_up(1024, DMA_WORD_PER_BEAT(sizeof(token_t)));
    }
    ...
    for (n = 0; n < ndev; n++) {
        ...
        for (coherence = ACC_COH_NONE; coherence <= ACC_COH_RECALL; coherence++) {
			...
			iowrite32(dev, PULP_CLUSTER_SPARE0_REG, spare0);
			iowrite32(dev, PULP_CLUSTER_SPARE1_REG, spare1);
